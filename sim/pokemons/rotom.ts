import {PokemonSet} from '../global-types';

export const rotomWash = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Choice Specs",
		ability: "Levitate",
		moves: ['Discharge', 'Volt Switch', 'Trick', 'Hydro Pump'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 252, spd: 0, spe: 4},
		ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomWash2 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Iapapa Berry",
		ability: "Levitate",
		moves: ['Nasty Plot', 'Volt Switch', 'Dark Pulse', 'Hydro Pump'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 164, spa: 92, spd: 0, spe: 0},
		ivs: {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomWash3 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Sitrus Berry",
		ability: "Levitate",
		moves: ['Nasty Plot', 'Discharge', 'Dark Pulse', 'Hydro Pump'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 236, spd: 0, spe: 20},
		ivs: {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomWash4 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Choice Specs",
		ability: "Levitate",
		moves: ['Discharge', 'Volt Switch', 'Dark Pulse', 'Hydro Pump'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 196, spd: 0, spe: 60},
		ivs: {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomWash5 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Sitrus Berry",
		ability: "Levitate",
		moves: ['Nasty Plot', 'Volt Switch', 'Dark Pulse', 'Hydro Pump'],
		nature: 'Bold',
		gender: '',
		evs: {hp: 252, atk: 0, def: 140, spa: 60, spd: 4, spe: 52},
		ivs: {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomWash6 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Wash",
		item: "Sitrus Berry",
		ability: "Levitate",
		moves: ['Nasty Plot', 'Volt Switch', 'Dark Pulse', 'Hydro Pump'],
		nature: 'Bold',
		gender: '',
		evs: {hp: 252, atk: 0, def: 244, spa: 4, spd: 4, spe: 4},
		ivs: {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomHeat = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Heat",
		item: "Assault Vest",
		ability: "Levitate",
		moves: ['Discharge', 'Volt Switch', 'Dark Pulse', 'Overheat'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 252, spd: 0, spe: 4},
		ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomHeat2 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Heat",
		item: "Assault Vest",
		ability: "Levitate",
		moves: ['Discharge', 'Volt Switch', 'Foul Play', 'Overheat'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 252, spd: 0, spe: 4},
		ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomCut = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Cut",
		item: "Choice Specs",
		ability: "Levitate",
		moves: ['Thunderbolt', 'Volt Switch', 'Leaf Storm', 'Shadow Ball'],
		nature: 'Modest',
		gender: '',
		evs: {hp: 252, atk: 0, def: 0, spa: 252, spd: 0, spe: 4},
		ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};

export const rotomCut2 = (): PokemonSet => {
	return {
		name: "Rotom",
		species: "Rotom-Cut",
		item: "Sitrus Berry",
		ability: "Levitate",
		moves: ['Nasty Plot', 'Discharge', 'Dark Pulse', 'Leaf Storm'],
		nature: 'Bold',
		gender: '',
		evs: {hp: 252, atk: 0, def: 92, spa: 140, spd: 4, spe: 20},
		ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31},
		level: 50,
	}
};
